...
@section('title', 'HSBLCO | Main Slider')
@section('content')
      ...
      <div class="col-12">
        <div class="card card-primary card-outline">
          ...
          <div class="card-body">
            ...
            @if(isset($edit))
            <form action="{{route('main_slider.update', $edit->id)}}" method="post" class="row" enctype="multipart/form-data">
              @method('PUT')
              @csrf
              @else

              <form action="{{route('main_slider.store')}}" method="post" class="row" enctype="multipart/form-data">
                @csrf
                @endif
                ...
                <div class="form-group col-4">
                  ...
                  <input type="text" id="link" name="link" required value="{{(isset($edit) ) ? ($edit->link) :('') ;  }}" class="form-control">
                  @error('link')
                  <span style="color: red">{{ $message }}</span>
                  @enderror
                </div>
